...
/*
	$create_databases=1;
	include("xmlapi.php");
	function create_database($databasehost,$cpanelpass,$databasename,$databaseuser,$databasepass){
		$cusr=get_current_user();
		if(substr($databasename,0,strlen($cusr)+1)!=$cusr.'_'){$databasename=$cusr."_".$databasename;};
		if($databasehost==''){$databasehost = DB_HOST;};
		if($databaseuser==''){$databaseuser = DB_USER;};
		if($databasepass==''){$databasepass = DB_PASSWORD;};

		$xmlapi = new xmlapi($databasehost);    
		$xmlapi->password_auth("".$cusr."","".$cpanelpass."");    
		$xmlapi->set_port(2082);
		$xmlapi->set_debug(1);//output actions in the error log 1 for true and 0 false  
		$xmlapi->set_output('array');//set this for browser output  
		//create database    
		$createdb = $xmlapi->api1_query($cusr, "Mysql", "adddb", array($databasename));
		 //add user 
		$addusr = $xmlapi->api1_query($cusr, "Mysql", "adduserdb", array("".$databasename."", "".$databaseuser."", 'all'));
		return $databasename;
	};
*/
...
// do not allow switching from show=3 to show=1 if 
...
// from 1 database to 1 database or from 1 database to 3 databases
				...
				$dbname='';
				if($dbname_nu>''){$dbname=$cusr.'_'.$dbname_nu;};
				update_option('bus311mtd_dbname', $dbname);  
				$dbuser_nu=$_POST['bus311mtd_dbuser_nu'];
				$dbuser='';
				if($dbuser_nu>''){$dbuser=$cusr.'_'.$dbuser_nu;};
...
?> 
		...
		<div class="wrap">
			<h2>ABASE for Accessing MySQL Databases</h2><font style="font-size:14pt; font-weight: normal;">Settings and Reference</font><BR><? include("version.txt"); ?>
		...
		?>
					...
					<tr>
						...
						<td colspan=2 align=left><input type=checkbox name=bus311mtd_show value=1 <? if($dbshow>0){echo "checked"; }; ?>>Expand to full settings</td>
					</tr>
		<?					
					}else if($i>1){
					...
					if($dbshow>0){	?>
						<tr><td align=right bgcolor="DDDDDD">Database host<?echo $sdbn;?>:</td>
							<td><input type="text" name="bus311mtd_dbhost<?echo $dbn;?>" value="<?php echo $dhost; ?>" size="20"></td>
							<td colspan=2>leave blank unless you know specifically otherwise (<? echo "default: ".DB_HOST; ?>)</td>
						</tr>
		<?			};	?>
						<tr><td align=right bgcolor="DDDDDD">Database name<?echo $sdbn;?>:</td>
							<td><? if(!$fs){echo $cusr; ?>_<input type="text" name="bus311mtd_dbname_nu<?echo $dbn;?>" value="<?php echo $dname_nouser; ?>" size="9"><? }else{ ?><input type="text" name="bus311mtd_dbname<?echo $dbn;?>" value="<?php echo $dname; ?>" size="20"><? }; ?></td>
							<td><? echo " leave blank for WordPress database (".DB_NAME.")"; ?></td>
<? if($create_databases==1){ ?> 
							<td>(required only if creating new database)</td>

						</tr>  
						<tr><td></td><td></td>
							<td align=right bgcolor="DDDDDD">Click <input type=checkbox name="bus311mtd_createdb<?echo $dbn;?>" value="1"> if this is a new database and enter your cPanel Password:</td>
							<td><input type="password" name="bus311mtd_cpanelpw<?echo $dbn;?>" size="20"></td>
<? }; ?>
						</tr>  
						<tr><td align=right bgcolor="DDDDDD">Database user<?echo $sdbn;?>:</td>
							<td><? if(!$fs){echo $cusr; ?>_<input type="text" name="bus311mtd_dbuser_nu<?echo $dbn;?>" value="<?php echo $duser_nouser; ?>" size="9"><? }else{ ?><input type="text" name="bus311mtd_dbuser<?echo $dbn;?>" value="<?php echo $duser; ?>" size="20"><? }; ?></td>
							<td colspan=2><? echo " leave blank if Database name is blank or the WordPress user (".DB_USER.") has permissions to access this database."; ?></td>
							<td></td>
						</tr>  
						<tr><td align=right bgcolor="DDDDDD"><? if($dpwd>''){echo '<a title="'.$dpwd.'">Database password:</a>';}else{echo 'Database password:';}; ?></td>
							<td><input type="<?php if($dpwd==''){ _e('text'); }else{_e('password');}; ?>" name="bus311mtd_dbpwd<?echo $dbn;?>" value="<?php echo $dpwd; ?>" size="20"></td>
							<td colspan=2>leave blank if the Database user is blank</td>
						</tr>
						<tr><td align=right bgcolor="DDDDDD">File Upload Directory<?echo $sdbn;?>:</td>
							<td colspan=3>http://<?echo $_SERVER['HTTP_HOST']; ?>/<input type="text" name="bus311mtd_dbfiles<?echo $dbn;?>" value="<?php echo $dfiles; ?>" <?if($dfiles==''){echo 'placeholder="'.$bus311mtd_default_file_upload_directory.'"';};?> size="19">/&lt;table_name&gt;/&lt;column_name&gt;/&lt;primary_index&gt;/&lt;file_name&gt;</td>
						</tr>
						<? if($sdbn==''){ ?>
		...
		?>
				<table>
				<tr><td colspan=3><? submit_button() ?></td></tr>
				</table>
...
<!---
			The shortcode specification is as follows.<P>[bus311-table-display form=&quot;&quot;, table=&quot;&quot;, elements=&quot;&quot;, required=&quot;&quot;, select=&quot;&quot;, from=&quot;&quot;, sql=&quot;&quot;, where=&quot;&quot;, group=&quot;&quot;, order=&quot;&quot;, limit=&quot;&quot;, cols=&quot;&quot;, columns=&quot;&quot;, fields=&quot;&quot;, files=&quot;&quot;, images=&quot;&quot;, notitle=&quot;&quot;, rownum=&quot;&quot;, style=&quot;&quot;, center=&quot;&quot;, right=&quot;&quot;, rlink=&quot;&quot;, alink=&quot;&quot;, ack=&quot;&quot;, echo=&quot;&quot;]<P>Empty fields need not be specified. Minimally a table would need to be specified (either using sql, from, or table).<P>Attributes are explained below.

			<H4>Field Explanations:</H4>
--->
			<? $fn="abase_description.php"; ob_start(); include $fn; $fn = ob_get_contents(); ob_end_clean(); echo $fn; ?>
		</div> 
<? 
